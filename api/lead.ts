...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/pressure-washing\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Pressure Washing`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Pressure Washing',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Pressure Washing Alerts <alerts@mursenmaintenance.com>',
